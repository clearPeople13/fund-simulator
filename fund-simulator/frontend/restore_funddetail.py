# -*- coding: utf-8 -*-
"""FundDetail.vue 全文反混淆：c→o 混淆（script/template/style 全覆盖）。
词典：dist chunks + 内置词表。规则：token 在词典→不动；否则尝试把部分 o 还原为 c，取词典中最优候选。
"""
import re, os
from itertools import combinations

ROOT = r"C:\Users\jiancent\WorkBuddy\fund\fund-simulator\frontend"
SRC = os.path.join(ROOT, "src", "views", "FundDetail.vue")
...
outlinestyle outlinecolor outlineoffset boxshadow inset outset
...
fundinfo fundcode fundname fundtype fundmanager inceptiondate establisheddate
benchmark navhistory unitnav accnav dailynetvalue growthrate accumulatenav
manager scale size fees rate fee rate risk level style strategy industry
sector theme concept position holdings composition allocation diversification
fundnav navlist navdate netvalue dateformat currency thousand separator
percentformat parsefloat numberformat tofixed round floor ceil abs max min
chart option series xaxis yaxis tooltip legend grid axisline splitline
axislabel areastyle linestyle itemstyle bordercolor backgroundcolor
cardbody cardheader cardfooter cardshadow infolist infoitem infolabel
infovalue actions btn button primary plain text danger warning success info
small medium large circle round square dashed solid dotted shadow hover
focus active disabled readonly selected checked loading empty placeholder
""".split()
for w in EXTRA:
...
def best_fix(tok):
    if tok in vocab:
        return tok
    idxs = [i for i, ch in enumerate(tok) if ch == "o"]
    if not idxs:
        return tok
    cands = {}
    for k in range(1, min(len(idxs), 2) + 1):
...
open(SRC + ".confused.bak", "w", encoding="utf-8").write(raw)
print("已备份 ->", SRC + ".confused.bak")
