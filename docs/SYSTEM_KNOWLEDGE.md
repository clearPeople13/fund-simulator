...
>
> 。
> 更新：2026-09-20 21:00 ｜ 应用：fund-simulator（127.0.0.1:3000，部署中心 127.0.0.1:8900 托管）
...
1. **每日复盘时间修正**：从 15:00 收盘分析时生成 → 改为 21:30 净值同步完成后生成（确保当日净值已公布）
2. **每日复盘内容丰富**：加持仓表现（领涨/领跌基金）+ 热点方向（当日最热板块）
3. **代码规范升级**：CODING_STANDARDS.md 加 5-agent 审核规则（基金经理/功能/代码/UI/交互）
4. **UI 优化**：卡片间距 16→20px、图表全局自适应（window resize 时所有 ECharts resize）、logo 升级（柱状图+上升箭头 SVG）、favicon 更新
5. **目录重构**：后端收拢到 backend/，前端保持 frontend/，前后端平级分离（用户明确拒绝 Monorepo apps/api + apps/web 方案）
