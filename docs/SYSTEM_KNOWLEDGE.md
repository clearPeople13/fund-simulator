...
>
> 。
> 更新：2026-09-21 10:30 ｜ 应用：fund-simulator（127.0.0.1:3000，部署中心 127.0.0.1:8900 托管）
...
> **2026-09-21 更新摘要**：
> - 实时分析间隔：30分钟 → 60分钟（整点触发）
> - 时间统一格式化：数据库存时间戳（毫秒），后端 `fmtDT()` 转北京时间字符串给前端
> - AI 自动交易走 order-engine（T+1 订单确认），不再直接写 transactions/holdings
> - 交易时间硬校验：非交易时间（周末/收盘后）AI 只分析不交易
> - 飞书通知：详细排版（基金代码+名称+净值+涨跌幅+建议金额+原因）+ 开始/结束/耗时时间（自动格式化秒/分/时）
> - 数据清洗：删除 aggressive 9/19 20:00 非交易时间违规交易，重算 holdings
> - 收益明细 bug 修复：transaction_date 时间戳转日期，不再从2002年开始
